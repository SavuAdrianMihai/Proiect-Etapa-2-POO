...
public class GreenStrategy implements ChoosingProducerStrategy {
    @Override
    public final void searchProducer(Input input, Distributor distributor) {
        for (int i = 0; i < input.getProducersData().size(); i++) {
            ...
            if (distributor.getEnergyNeededCurrently() < input.getProducersData().get(i).
                ...
                distributor.setEnergyNeededCurrently(distributor.getEnergyNeededCurrently() - input.
                        getProducersData().get(i).getEnergyPerDistributor());
            }
